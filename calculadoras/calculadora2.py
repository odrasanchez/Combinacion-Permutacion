...
# Main, aquí se encuentra la solicitud de datos y llamado de funciones
def main():
    ...
    if op == 1:
        ...
        n = int (input('Ingrese el valor total de elementos:'))
        r = int (input('Ingrese el valor de elementos tomados:'))
        if r == 0:
            return print("El valor no es valido")
        # Llamando a la funcion con las variables recibidas
        opertations_permuPR(n,r)
...
def operations_comb (n, m):
    ...
    # Se envian las variables con las operaciones previas
...
def opertations_permuPR(n,r):
    producto = 1
    
    for i in range (0, r):
        num = int(input("Ingrese el valor %i: " %i))
        producto *= math.factorial(num)
        
    resul = math.factorial(n) / producto
    format_permutR(n, producto, resul)
